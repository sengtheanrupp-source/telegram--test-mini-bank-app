...
function softConfirm(message, onOk) {
  ...
  const text = document.getElementById("softConfirmText");
  ...
  feed.innerHTML = posts
    .map((p) => {
      ...
      return (
        ...
        "</div>" +
        (p.caption
          ? '<p class="px-3.5 pb-2 text-sm font-bold text-slate-800 dark:text-slate-100 whitespace-pre-wrap leading-relaxed khmer-text" lang="km" dir="auto" style="font-family:\'Noto Sans Khmer\',\'Plus Jakarta Sans\',sans-serif">' +
...
function renderVoiceBubble(c) {
  ...
  return (
    ...
    "</span></button>" +
    (c.text
      ? '<p class="text-[12px] font-bold text-slate-700 dark:text-slate-200 mt-1 khmer-text" lang="km" dir="auto" style="font-family:\'Noto Sans Khmer\',sans-serif">' +
        escapeHtml(c.text) +
        "</p>"
      : "")
  );
  ...
  list.innerHTML = comments
    .map((c) => {
      ...
      return (
        ...
        (c.voiceData || c.voice
          ? renderVoiceBubble(c)
          : '<p class="text-sm font-bold text-slate-900 dark:text-slate-100 mt-0.5 khmer-text" lang="km" dir="auto" style="font-family:\'Noto Sans Khmer\',sans-serif">' +
            escapeHtml(c.text || "") +
            "</p>") +
...
function stopVoiceComment(discard) {
  if (_voiceRecorder && _voiceRecording) {
    try {
      _voiceRecorder.stop();
    } catch (e) {}
  }
...
function _ssStopFrameLoop() {
  if (_ssFrameTimer) {
    clearTimeout(_ssFrameTimer);
    _ssFrameTimer = null;
  }
}

function _ssHandleViewerData(raw) {
  try {
...
function setVoiceSessionUI(opts) {
  opts = opts || {};
...
function openVoiceSessionModal() {
  const m = document.getElementById("voiceSessionModal");
...
function stopVoiceSessionMic() {
  ...
  try {
    if (_voiceVadRaf) {
      cancelAnimationFrame(_voiceVadRaf);
      _voiceVadRaf = null;
    }
  } catch (e) {}
  try {
...
function startVoiceSessionTimer() {
  _voiceSessionSeconds = 0;
  clearInterval(_voiceSessionTimerIv);
  setVoiceSessionUI({ seconds: 0 });
  ...
  try {
    ...
    if (bankModal && !bankModal.classList.contains("hidden") && doneBtn) {
      return {
        ...
        label: "Done / return",
        run: () => {
          if (typeof handlePaymentDoneAction === "function")
    ...
    if (
      payView &&
      !payView.classList.contains("hidden") &&
      (workflowState.link_token || workflowState.payment_token)
    ) {
      return {
        id: "deeplink_pay",
        label: "Confirm deeplink pay",
        run: () => {
          unlockAudioEngine();
    ...
    try {
      if (typeof speakKhmerAudioFallback === "function") {
        speakKhmerAudioFallback(
          "សូមបើកទំព័រទូទាត់ រួចនិយាយដើម្បីបញ្ជាក់។",
        );
      }
    } catch (e) {}
    return;
...
async function startVoiceSessionMic() {
  ...
  if (!SR) {
    setVoiceSessionUI({
      status: "Speech not supported",
      assistant: "Use on-screen Pay buttons",
      listening: false,
    });
    return;
  }
