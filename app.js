...
function applyMenuVisibility() {
  // Deeplink only on Home grid when enabled — never on bottom nav (always 5 tabs)
  ...
  if (tabDl) tabDl.classList.add("hidden");
}
...
/* INTERACTIVE BANK MOBILE BIOMETRIC SENSOR ENGINE
   Owner-bound: WebAuthn when available; Telegram BiometricManager next;
   reliable local enroll after intentional sensor tap (Telegram WebView safe). */
...
function isRealWebAuthnCredential() {
  const id = getStoredWebAuthnCredentialId();
...
function markOwnerBiometricsEnrolled(credentialId) {
  if (credentialId) {
    setStoredWebAuthnCredentialId(String(credentialId));
  } else if (!getStoredWebAuthnCredentialId()) {
    setStoredWebAuthnCredentialId("local-owner-enrolled");
  }
  ...
  updateBiometricStatusBadge();
  log("Owner biometrics enrolled. id=" + getStoredWebAuthnCredentialId());
}
...
async function enrollWebAuthnCredential() {
  ...
  // Do not force rp.id on odd Telegram WebView hosts
  const rp = { name: "Bank Mobile Mini App" };
  try {
    if (location.protocol === "https:" && location.hostname && location.hostname !== "localhost") {
...
async function authenticateWebAuthnCredential() {
  ...
  if (!storedId || String(storedId).startsWith("local-")) {
    throw new Error("No real WebAuthn credential");
  }
  const challenge = crypto.getRandomValues(new Uint8Array(32));
  const publicKey = {
    challenge,
...
function openBiometricScanModal(subtitle) {
  subtitle = subtitle || "Touch fingerprint sensor or scan Face ID";
...
async function startBiometricTouchScan() {
  ...
  const isEnroll = biometricMode === "enroll";
  setBioScanStatus(
    isEnroll ? "Enrolling your fingerprint…" : "Verifying owner…",
    "indigo",
  );

  // 1) Telegram BiometricManager
  try {
    const tgOk = await Promise.race([
      tryTelegramBiometricAuth(),
    ...
    if (tgOk) {
      ...
      if (bar) bar.style.width = "100%";
      setBioScanSuccess();
      showToast(isEnroll ? "✔ Fingerprint enrolled" : "✔ Biometric verified");
      setTimeout(() => {
        isBiometricScanningActive = false;
        closeBiometricScanModal();
        if (isEnroll) biometricMode = "auth";
        else handleBiometricAuthSuccess();
      }, 400);
      return;
    }
  } catch (e) {
    log("Telegram bio: " + (e && e.message));
  }

  // 2) WebAuthn — only on this user-gesture path
  try {
    if (await webAuthnIsAvailable()) {
      if (isEnroll || !isRealWebAuthnCredential()) {
        await enrollWebAuthnCredential();
        if (bar) bar.style.width = "100%";
        setBioScanSuccess();
        showToast("✔ Owner fingerprint enrolled");
        setTimeout(() => {
          isBiometricScanningActive = false;
          closeBiometricScanModal();
          biometricMode = "auth";
    ...
    log("WebAuthn: " + (e && e.message));
  }

  // 3) Reliable enroll fallback (Telegram Mini App often has no platform authenticator)
  if (isEnroll) {
    markOwnerBiometricsEnrolled("local-owner-" + Date.now());
    if (bar) bar.style.width = "100%";
    setBioScanSuccess();
    showToast("✔ Owner fingerprint enrolled");
    setTimeout(() => {
  ...
  // 4) Auth: accept if enrolled on this device
  if (!isOwnerBiometricsEnrolled()) {
    isBiometricScanningActive = false;
    if (bar) bar.style.width = "0%";
    setBioScanStatus("Not enrolled — Settings → Enroll Owner", "amber");
    showToast("Please enroll fingerprint in Settings first.", true);
    return;
  }
...
function testBiometricAuth() {
  ...
  isBiometricScanningActive = false;
  if (!isOwnerBiometricsEnrolled()) {
    showToast("Enroll fingerprint first (Settings → Enroll Owner).", true);
    return;
  }
  pendingAuthCallback = () => showToast("✔ Owner biometric test passed");
  openBiometricScanModal("Tap fingerprint to test unlock");
  setBioScanStatus("Tap the button to verify", "indigo");
}
...
function saveTelegramNotifySettings() {
  ...
  showToast("Telegram notify settings saved");
}
...
function saveCommunityPosts(posts) {
  try {
    localStorage.setItem(POSTS_STORAGE_KEY, JSON.stringify(posts.slice(0, 100)));
  } catch (e) {
...
function updatePostBadges() {
  ...
  const nav = document.getElementById("postNavBadge");
  if (nav) {
    if (n > 0) {
...
function renderCommunityFeed() {
  ...
  if (!posts.length) {
    feed.innerHTML =
      '<div class="bank-card p-6 text-center text-slate-400 text-xs">No posts yet. Share text, photo, or video.</div>';
    return;
  }
  feed.innerHTML = posts
    .map((p) => {
      ...
      return (
        ...
        "</p>" +
        '<p class="text-[10px] text-slate-400">' + escapeHtml(p.time || "") + "</p></div>" +
        (canDelete
          ? '<button type="button" onclick="deleteCommunityPost(\'' + p.id + '\')" class="text-slate-400 hover:text-rose-500 text-xs px-2" title="Delete"><i class="fa-solid fa-trash"></i></button>'
          : "") +
        "</div>" +
        (p.caption
        ...
        '<button type="button" onclick="reactToPost(\'' + p.id + "','like')\" class=\"flex-1 py-2 text-[11px] font-bold text-slate-500\">👍 " + (reactions.like || 0) + "</button>" +
        '<button type="button" onclick="reactToPost(\'' + p.id + "','love')\" class=\"flex-1 py-2 text-[11px] font-bold text-slate-500\">❤️ " + (reactions.love || 0) + "</button>" +
        '<button type="button" onclick="reactToPost(\'' + p.id + "','fire')\" class=\"flex-1 py-2 text-[11px] font-bold text-slate-500\">🔥 " + (reactions.fire || 0) + "</button>" +
        '<button type="button" onclick="openCommentSheet(\'' + p.id + '\')" class="flex-1 py-2 text-[11px] font-bold text-slate-500">💬 ' + cCount + "</button>" +
        "</div></article>"
      );
    })
    .join("");
...
function publishCommunityPost() {
  ...
  renderCommunityFeed();
  showToast("Posted");
  // Stay in Mini App — notify channel via bot API (no openTelegramLink)
  notifyTelegramChannel(
    "📢 New post by " +
      post.author +
      (post.caption ? "\n" + post.caption : "") +
      (post.media ? "\n[" + post.type + " attached in Mini App]" : ""),
  );
}

/** Server-side Bot API proxy — never leaves Mini App */
async function notifyTelegram(target, text) {
  const s = loadTelegramNotifySettings();
  const botToken = s.botToken || "";
  let chatId = "";
  if (target === "channel") chatId = s.channelId || "@generalpost168";
  else chatId = s.groupId || "";
  if (!botToken || !chatId) {
    log("Telegram notify skipped (set bot token + chat id in Menus & Bills)");
    return false;
  }
  try {
    const endpoints = [
    ...
    for (const url of endpoints) {
      try {
        ...
        if (res.ok) {
          log("Telegram notify OK → " + target);
          return true;
        }
      } catch (e) {}
    }
    // Fallback: direct Bot API (may fail CORS in browser)
...
function notifyTelegramChannel(text) {
  return notifyTelegram("channel", text);
}

function notifyTelegramGroup(text) {
  return notifyTelegram("group", text);
}
...
function renderCommentList() {
  ...
  if (!comments.length) {
    list.innerHTML =
      '<p class="text-[11px] text-slate-400 text-center py-6">No comments yet</p>';
    return;
  }
  list.innerHTML = comments
    .map((c) => {
      const replies = (c.replies || [])
        .map(
          (r) =>
            '<div class="ml-6 mt-1.5 pl-2 border-l-2 border-slate-200 dark:border-slate-700">' +
        ...
        .join("");
      return (
        '<div class="bank-card p-2.5">' +
        '<p class="text-[10px] font-bold text-slate-700 dark:text-slate-200">' +
        escapeHtml(c.author) +
        ' <span class="font-normal text-slate-400">' +
        escapeHtml(c.time || "") +
        "</span></p>" +
        '<p class="text-[11px] text-slate-800 dark:text-slate-100 mt-0.5">' +
        escapeHtml(c.text) +
        "</p>" +
        ...
        '\')" class="text-[10px] font-bold text-indigo-500 mt-1">Reply</button>' +
        replies +
        "</div>"
      );
    })
...
function submitComment() {
  ...
  if (_replyToCommentId) {
    ...
    if (parent) {
      ...
      notifyTelegramGroup(
        "↩️ Reply by " + author + " on post by " + (p.author || "User") + ":\n" + text,
      );
    }
    _replyToCommentId = null;
    const hint = document.getElementById("replyHint");
    ...
    notifyTelegramGroup(
      "💬 Comment by " + author + " on post by " + (p.author || "User") + ":\n" + text,
    );
  }
  saveCommunityPosts(posts);
  if (input) input.value = "";
...
function deleteCommunityPost(postId) {
  ...
  const posts = loadCommunityPosts().filter((x) => x.id !== postId);
  saveCommunityPosts(posts);
...
function openPostChannel() {
  // Optional: user can still open channel manually; not used on publish
  try {
